...
insert into flight_data (id, today, tomorrow) values (1, '', '')
on conflict (id) do nothing;
...
-- ⚠️ 如果你【之前已经建过旧版 flight_data 表】（只有 content 一列），
-- 不要删表重建，直接跑下面这几句把列补上即可：
--   alter table flight_data add column if not exists today text not null default '';
--   alter table flight_data add column if not exists tomorrow text not null default '';
-- 旧数据会保留在 content 列，新版代码不会读它，可以留着。
...
| 看航班信息 | ✅ | ✅ |
| 拉取管理员数据 | ✅ 点"同步管理员数据" | — |
| 推送航班数据 | ❌ 数据库拒绝 | ✅ 编辑后点"同步给所有用户" |
| 改机型筛选 | ✅ 存自己 | ✅ 存自己 |
| 每日首次弹公告 | ✅ | ✅ |

> **今日 / 明日数据独立**：`flight_data` 表有 `today`、`tomorrow` 两列，
> 管理员点"同步"时只更新当前所选那一天，另一天不受影响；
> 普通用户点"同步管理员数据"时也只拉取当前这一天。
