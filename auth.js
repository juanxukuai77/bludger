...
function updateAvatar() {
  const client = window.supabaseClient
  const el = document.getElementById('userAvatarText')
  if (!el) return
  if (!client) { el.textContent = 'U'; return }
  client.auth.getUser().then(({ data }) => {
    const email = data?.user?.email || ''
    el.textContent = (email.charAt(0) || 'U').toUpperCase()
  })
}
...
async function checkAdmin() {
  ...
  // 管理员才显示公告编辑按钮 + 航班信息编辑区
  const adminBtn = document.getElementById('admin-announce-btn')
  if (adminBtn) adminBtn.style.display = isAdmin ? 'inline-block' : 'none'

  const flightInput = document.getElementById('flight-info-input-wrap')
  if (flightInput) flightInput.style.display = isAdmin ? 'block' : 'none'
...
async function loadFlightData() {
  ...
  // 管理员：直接显示服务器内容，可编辑
  // 普通用户：优先显示自己本地改过的，否则显示管理员同步的
  const show = isAdmin
    ? serverContent
    : (localContent.trim() ? localContent : serverContent)

  if (display) display.value = show

  const input = document.getElementById('flight-info-input')
  if (input && isAdmin) input.value = serverContent
}
...
function saveFlightLocal() {
  ...
  localStorage.setItem(FLIGHT_KEY, display.value)
  setFlightStatus('✅ 已保存到本地（不影响其他用户）', 'success')
  return false
}
...
async function saveFlightData() {
  const client = window.supabaseClient
  if (!client) { alert('客户端未初始化'); return false }
  if (!isAdmin) { alert('仅管理员可同步航班信息'); return false }
  ...
  if (error) {
    setFlightStatus('保存失败：' + error.message, 'error')
  } else {
    // 管理员自己也清掉本地覆盖，统一走服务器
    localStorage.removeItem(FLIGHT_KEY)
    const display = document.getElementById('flight-info-display')
    if (display) display.value = text
    setFlightStatus('✅ 已保存，所有用户已自动同步！', 'success')
  }
  return false
}
